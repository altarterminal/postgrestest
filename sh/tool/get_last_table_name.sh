...
DEVICE_SCHEMA_PREFIX="${COMMON_DEVICE_SCHEMA_PREFIX}"
DEVICE_SCHEMA_SUFFIX="${COMMON_DEVICE_SCHEMA_SUFFIX}"

EVALDATA_TABLE_PREFIX="${COMMON_EVALDATA_TABLE_PREFIX}"
EVALDATA_TABLE_SUFFIX="${COMMON_EVALDATA_TABLE_SUFFIX}"

DEVICE_SCHEMA_NAME="${DEVICE_SCHEMA_PREFIX}_${DEVICE_NAME}_${DEVICE_SCHEMA_SUFFIX}"

EVALDATA_TABLE_NAME_BODY="${PROJECT_NAME}_${PROJECT_VERSION}_${DEVICE_NAME}_[0-9][0-9]"
EVALDATA_TABLE_NAME_PATTERN="${EVALDATA_TABLE_PREFIX}_${EVALDATA_TABLE_NAME_BODY}_${EVALDATA_TABLE_SUFFIX}"
...
target_table_name=$(
  printf '%s\n' "${table_list}" |
    awk -F, '{ print $2; }' |
    grep "^${EVALDATA_TABLE_NAME_PATTERN}$" |
    sort |
    tail -n 1
)
